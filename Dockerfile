...
# cmake
RUN rm -fr /app/build/ && cmake -DCMAKE_BUILD_TYPE=Debug -DDynamoRIO_DIR=/app/DynamoRIO-Linux-9.0.1/cmake  -S  . -B ./build
# build
RUN cd ./build && make VERBOSE=1

# ENTRYPOINT /app/DynamoRIO-Linux-9.0.1/bin64/drrun -c ./build/libhello_world_client.so -- /usr/bin/ls
